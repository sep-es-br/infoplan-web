...
export class OrgChartVerticalComponent implements OnInit, OnChanges {
  ...
  private getSeriesName(chart: IChartOptions, index: number): string {
    if (
      chart &&
      chart.data &&
      chart.data.datasets &&
      ...
      return chart.data.datasets[index].label;
    }
    return ""; // Retorna uma string vazia ou um nome padrão se não houver dados
  }
}
