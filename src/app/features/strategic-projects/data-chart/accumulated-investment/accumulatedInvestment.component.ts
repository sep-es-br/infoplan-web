...
export class AccumulatedInvestmentComponent implements OnChanges {
  ...
  assembleFlipTableContent(rawData: IStrategicProjectAccumulatedInvestment[], shouldStartExpanded: boolean = false) {
      const tableColumns = [
        ...
        { propertyName: 'custoPrevistoAcumulado', displayName: 'Previsto Acumulado' },
        { propertyName: 'custoRealizado', displayName: 'Realizado' },
        { propertyName: 'custoRealizadoAcumulado', displayName: 'Realizado Acumulado' },
      ];

      const finalData: Array<TreeNode> = rawData.map((investimento) => ({
        data: [
          { originalPropertyName: 'anoMes', propertyName: 'firstColumn', value: `${this.formatDate(investimento.anoMes, 'year-month')}` },
          { propertyName: 'custoPrevisto', value: this.utilitiesService.formatCurrencyString(investimento.custoPrevisto) },
          { propertyName: 'custoPrevistoAcumulado', value: this.utilitiesService.formatCurrencyString(investimento.custoPrevistoAcumulado) },
          { propertyName: 'custoRealizado', value: this.utilitiesService.formatCurrencyString(investimento.custoRealizado) },
          { propertyName: 'custoRealizadoAcumulado', value: this.utilitiesService.formatCurrencyString(investimento.custoRealizadoAcumulado) },
        ],
        children: [],
    ...
    handleUserTableSearch(searchTerm: string) {
      if (searchTerm.length > 0) {
        ...
        const filteredItems = this.accumulatedInvestmentData.filter((investimento) => (
          ...
          this.utilitiesService.formatCurrencyString(investimento.custoPrevistoAcumulado).includes(preparedSearchTerm) ||
          this.utilitiesService.formatCurrencyString(investimento.custoRealizado).includes(preparedSearchTerm) ||
          this.utilitiesService.formatCurrencyString(investimento.custoRealizadoAcumulado).includes(preparedSearchTerm)
        ));
