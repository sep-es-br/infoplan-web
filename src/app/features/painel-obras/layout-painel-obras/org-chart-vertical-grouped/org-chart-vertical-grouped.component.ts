...
export class OrgChartVerticalGroupedComponent
  ...
  private updateChart() {
    ...
    // - when groupingMode refers to STATUS as primary (startsWith 'STATUS' or 'STATUS_'),
    //   the major should be the second part (status).
    // - otherwise the major defaults to the first part (municipio or year, etc.).
    const majorFromLabelIndex = (() => {
      const gm = (this.groupingMode || '').toString().toUpperCase();
      if (gm.startsWith('STATUS') || gm.startsWith('STATUS_') || gm === 'STATUS_MUNICIPIO') return 1;
      return 0;
    })();
    ...
    labelsRaw.forEach((label, idx) => {
      ...
      const majorVal = majorFromLabelIndex === 0 ? first : (second || first);
      const minorVal = majorFromLabelIndex === 0 ? (second || first) : first;
