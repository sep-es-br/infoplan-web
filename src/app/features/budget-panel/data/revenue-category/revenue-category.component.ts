...
@Component({
  selector: "ngx-revenue-category",
  templateUrl: "./revenue-category.component.html",
  styleUrls: ["./revenue-category.component.scss"],
  standalone: true,
  imports: [OrgChartHorizontalComponent, FlipTableComponent],
})
export class RevenueCategoryComponent implements OnChanges, OnDestroy {
  ...
  private receitaData: IRevenueCategoryBudgetExecutionResponse[] = [];
  ...
  private loadData(): void {
    ...
    this._painelService
      .getRevenueByCategory(this.filter)
  ...
  private processTable(): void {
    const anos = [...new Set(this.receitaData.map((item) => item.year))].sort(
      (a, b) => a - b,
    );
    const categorias = [
      ...new Set(this.receitaData.map((item) => item.category)),
    ];

    const treeNodes = categorias.map((categoria) => {
      const nodeData = [{ propertyName: "category", value: categoria }];

      anos.forEach((year) => {
        const dado = this.receitaData.find(
          (d) => d.category === categoria && d.year === year,
        );
        const valorReceitaLiquida = dado?.netRevenue || 0;
    ...
    const totalNodeData = [{ propertyName: "category", value: "Total" }];
    ...
    const defaultColumns = anos.map((year) => ({
      propertyName: `ano_${year}`,
      displayName: `Arrecadação Líquida - ${year} (R$)`,
      alignment: {
  ...
  private calcularVariacao(categoria: string, anos: number[]): number {
    ...
    const valorInicial =
      this.receitaData.find(
        (d) => d.category === categoria && d.year === primeiroAno,
      )?.netRevenue ?? 0;

    const valorFinal =
      this.receitaData.find(
        (d) => d.category === categoria && d.year === ultimoAno,
      )?.netRevenue ?? 0;
