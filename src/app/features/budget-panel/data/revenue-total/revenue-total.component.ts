...
@Component({
  selector: "ngx-revenue-total",
  templateUrl: "./revenue-total.component.html",
  styleUrls: ["./revenue-total.component.scss"],
  providers: [ShortNumberPipe],
})
export class RevenueTotalComponent implements OnChanges, OnDestroy {
  @Input() filter!: IBudgetExecutionRequest;
  @Output()
  dataRevenueTotalCards: EventEmitter<
    IRevenueTotalBudgetExecutionResponse | IRevenueTotalBudgetExecutionResponse[]
  > = new EventEmitter<
    IRevenueTotalBudgetExecutionResponse | IRevenueTotalBudgetExecutionResponse[]
  >();
  ...
  private responseData: IRevenueTotalBudgetExecutionResponse[] | null = null;
  ...
  private loadData(): void {
    ...
    this._painelService
      .getRevenueTotal(this.filter)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response) => {
          this.responseData = [response];
          this.processData(response);
          this._comunicationCardsService.sendRevenueTotal(response);
  ...
  private processData(dados: IRevenueTotalBudgetExecutionResponse): void {
    const dadosOrdenados = dados;
    this.chartData = {
      data: {
        labels: dados.year ? [dados.year.toString()] : [],
        datasets: [
          {
            label: "Previsão Inicial Líquida",
            data: [dados.plannedRevenueValue || 0],
            backgroundColor: this._chartProcessor.colors[0],
          },
          {
            label: "Arrecadação Líquida",
            data: [dados.netRevenueValue || 0],
  ...
  private processTableData(
    ...
    const year = dadosArray[0]?.year || 2025;

    const treeNodes = dadosArray.flatMap((item) => {
      const previsao = item.plannedRevenueValue || 0;
      const arrecadacao = item.netRevenueValue || 0;
    ...
    this.tableContent = {
      customColumn: {
        propertyName: "label",
        displayName: `Receita Realizada/Prevista - ${year}`,
        alignment: {
  ...
  handleTableSearch(query: string): void {
    ...
    const filtered = this.responseData.filter(
      (item: IRevenueTotalBudgetExecutionResponse) => {
        const yearStr = item.year?.toString() || "";
        const netRevenue =
          item.netRevenueValue
            ?.toLocaleString("pt-BR", { currency: "BRL", style: "currency" })
            .replace("R$", "")
            .trim() || "0";
        const receitaPrevista =
          item.plannedRevenueValue
            ?.toLocaleString("pt-BR", { currency: "BRL", style: "currency" })
            .replace("R$", "")
            .trim() || "0";

        const percentual =
          item.plannedRevenueValue > 0
            ? (
              (item.netRevenueValue / item.plannedRevenueValue) *
              100
            ).toFixed(2)
            : "0";

        return (
          yearStr.toLowerCase().includes(search) ||
          netRevenue.toLowerCase().includes(search) ||
