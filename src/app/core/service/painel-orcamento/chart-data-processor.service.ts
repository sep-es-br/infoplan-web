...
export class ChartDataProcessorService {
  ...
  private readonly DEFAULT_SORT_FIELD = "receitaLiquida";
  ...
  criarChartDespesaGndTotal(
    ...
    const dadosAnoAtual = data.filter((d) => d?.ano === anoAtual);
  ...
  private construirDatasetsGndTotal(
    ...
    const config = {
      ...
      campos: ["vlr_orcado", "vlr_autorizado", "vlr_empenhado", "vlr_liquidado", "vlr_pago_com_rap"],
  ...
  private criarDatasetsLiquidadoPago(
    ...
    const config = [
      { label: `Liquidado ${anoRecente}`, campo: "vlr_liquidado", ano: anoRecente, cor: cores[0] },
      { label: `Liquidado ${anoAnterior}`, campo: "vlr_liquidado", ano: anoAnterior, cor: cores[1] },
      { label: `Pago com RAP ${anoRecente}`, campo: "vlr_pago_com_rap", ano: anoRecente, cor: cores[2] },
      { label: `Pago com RAP ${anoAnterior}`, campo: "vlr_pago_com_rap", ano: anoAnterior, cor: cores[3] },
    ];
    ...
    return config.map(({ label, campo, ano, cor }) => ({
      ...
      data: categorias.map((categoria) => {
        const items = dados.filter((d) => d[campoLabel] === categoria && d.ano === ano);
  ...
  private criarChartAnoUnico(
    ...
    const dadosPrevisao = this.extrairDadosPorCategoria(dados, categorias, ano, campoLabel, ["vlr_receita_prevista"]);
    const dadosArrecadacao = this.extrairDadosPorCategoria(dados, categorias, ano, campoLabel, ["receitaLiquida", "vlr_receita_liquida"]);
  ...
  private criarDatasets(dados: any[], categorias: any[], anos: number[], campoLabel: string): any[] {
    return anos.map((ano, index) => ({
      label: `${ano}`,
      data: this.extrairDadosPorCategoria(dados, categorias, ano, campoLabel, ["receitaLiquida", "vlr_receita_liquida"]),
  ...
  private criarTreeData(
    ...
    return categorias.map((categoria) => {
      const nodeData = [
        ...
        ...anos.map((ano) => {
          const item = dados.find((d) => d[campoLabel] === categoria && d.ano === ano);
          return {
            originalPropertyName: `ano_${ano}`,
  ...
  private extrairAnos(dados: any[]): number[] {
    ...
    return [...new Set(dados.map((item) => item.ano))].filter((ano) => ano != null).sort();
  }
